...
func createDefaultHandler(t *testing.T, codec *wire.Codec) http.Handler {
	coreContext := context.NewCoreContextFromViper()
	router := mux.NewRouter()
	rpc.RegisterRoutes(coreContext, router)

	keyBase, err := keys.GetKeyBase() //XXX
	require.NoError(t, err)

	RegisterRoutes(coreContext, router, codec, keyBase)
	return router
}
...
func createGenesisDoc(t *testing.T, genesisFile string,
	...
	for _, gdValidator := range genesisDoc.Validators {
		pk := gdValidator.PubKey

		appGenTx, _, _, err := gapp.GaiaAppGenTxNF(codec, pk,
			sdk.AccAddress(pk.Address()), "moniker")
