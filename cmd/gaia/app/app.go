...
func NewGaiaApp(logger log.Logger, db dbm.DB, traceStore io.Writer, loadLatest bool, baseAppOptions ...func(*bam.BaseApp)) *GaiaApp {
	...
	app.sentinelKeeper = sent.NewKeeper(
		app.cdc,
		app.keySentinel,
		app.bankKeeper,
		app.accountKeeper,
		sent.DefaultCodeSpace,
	)
	...
	app.QueryRouter().
		...
		//AddRoute("sentinel", sent.NewQuerier(app.sentinelKeeper)),

	// initialize BaseApp
	app.MountStores(app.keyMain, app.keyAccount, app.keyStaking, app.keyMint, app.keyDistr,
		app.keySlashing, app.keyGov, app.keyFeeCollection, app.keyParams,app.keySentinel)
